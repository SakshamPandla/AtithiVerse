...
function initializeSearch() {
    ...
    // Attach live Google Custom Search suggestions (cities/places)
    // Debounced fetch to backend proxy /api/google-search
    let suggestionBox;
    if (searchDestination) {
        ...
        searchDestination.addEventListener('input', function() {
            ...
            if (!q) { suggestionBox.style.display = 'none'; suggestionBox.innerHTML=''; return; }
            debounceTimer = setTimeout(async () => {
                try {
                    const resp = await fetch(`/api/google-search?q=${encodeURIComponent(q)}&num=6`);
                    const data = await resp.json();
                    if (!data.success) { suggestionBox.style.display='none'; return; }
                    const items = (data.results || []).slice(0,6);
                    if (!items.length) { suggestionBox.style.display='none'; return; }
                    suggestionBox.innerHTML = items.map(it => `
                        <div class="suggestion-item" data-value="${(it.title || '').replace(/"/g,'&quot;')}">
                            <i class="fas fa-location-dot me-2"></i>${it.title}
                            <small class="text-muted d-block">${it.displayLink || ''}</small>
                        </div>
                    `).join('');
        ...
        suggestionBox.addEventListener('click', function(e){
            ...
            if (val) searchDestination.value = val;
            suggestionBox.style.display='none';
        });
...
document.addEventListener('DOMContentLoaded', function() {
    // Load weather for the first option (Agra) by default
    setTimeout(() => {
